...
def run_swmm(valve_set1, valve_set2, valve_set3):
...
un_data, un_overflow = run_swmm(1.0, 1.0, 1.0)
...
def fc(x):
    data, overflow  = run_swmm(x[0][0], x[0][1], x[0][2])
    ...
    for i in overflow.keys():
        d += overflow[i]
    loss = distance1 + distance2 + distance3 + (d)*10.0
...
c_data, c_overflow = run_swmm(myBopt.x_opt[0], myBopt.x_opt[1], myBopt.x_opt[2])
