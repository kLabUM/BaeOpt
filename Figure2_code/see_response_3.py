...
actions = []
for ind in index:
    temp_myBopt.__dict__.update(pickle.load(open("./3_pond_fig2/Bayopt"+str(ind)+".pickle", 'rb')))
    temp_myBopt._compute_results()
    data_controlled = swmm_execute(temp_myBopt.x_opt.reshape(1,3))
    perf = performance_metric(data_controlled, THRESHOLD)
    print(perf)
    pre.append(perf)
    actions.append(temp_myBopt.x_opt.reshape(1,3))

# Save the info
data_trained = {"iter":index, "performance":pre, "actions":actions}
np.save("./3_pond_fig2/performance_bayopt_3", data_trained)
